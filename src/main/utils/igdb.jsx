...
class IGDB {
  ...
  async getGameInfo(game) {
    ...
    if (this.client_id === undefined)
      throw new Error("No client ID registered");
    ...
    if (gameDetails.length === 0)
      return { code: 404, error: "Could not find game in Igdb." };

    // Fetch cover
    let covers = await this.#fetchFromIGDB(
      `fields *; where game = ${gameDetails[0].id};`,
      "covers"
    );
    let cover_base_url = covers[0].url.replace("//", "https://");

    let web_id = gameDetails[0].websites[0];
    let websites = web_id
      ? await this.#fetchFromIGDB(
          `fields *; where id = (${gameDetails[0].websites.join(", ")});`,
          "websites"
        )
      : undefined;
    ...
    return {
      ...
      name: gameDetails[0]?.name,
      summary: gameDetails[0]?.summary,
      storyline: gameDetails[0]?.storyline,
      rating: gameDetails[0]?.rating,
      websites: {
        igdb: gameDetails[0]?.url,
        developer:
          websites.filter((w) => w.category === 1)[0]?.url || undefined,
        steam: websites.filter((w) => w.category === 13)[0]?.url || undefined,
        epicgames:
          websites.filter((w) => w.category === 16)[0]?.url || undefined,
        youtube: websites.filter((w) => w.category === 9)[0]?.url || undefined,
        twitter: websites.filter((w) => w.category === 5)[0]?.url || undefined,
        instagram:
          websites.filter((w) => w.category === 8)[0]?.url || undefined,
      },
    };
  }
  ...
  async getGameCovers(game) {
    ...
    let cover_base_url = covers[0].url.replace("//", "https://");
    covers_arr.push(cover_base_url.replace("t_thumb", `t_cover_big`));

    // Fetch screenshots
    let screenshots = await this.#fetchFromIGDB(
      `fields screenshots.*; where id = ${gameDetails[0].id};`
    );
    if (screenshots[0]?.screenshots !== undefined)
      screenshots[0].screenshots.forEach((i) => {
        const url = i.url.replace("//", "https://");
        covers_arr.push(url.replace("t_thumb", `t_cover_big`));
      });
